...
elseif (strlen( $_POST['password_hash']) > 20 || strlen($_POST['password_hash']) < 4)
{
...
elseif (ctype_alnum($_POST['password_hash']) != true)
{
        /*** if there is no match ***/
...
else
{
    ...
    $password_hash = filter_var($_POST['password_hash'], FILTER_SANITIZE_STRING);
    ...
    include_once("db_connect.php");
     ...
     try {
        ...
        $stmt = $dbh->prepare("SELECT email, password_hash FROM account
                                 WHERE email = :email AND
                                       password_hash = :password_hash");

        /*** bind the parameters ***/
        $stmt->bindParam(':email', $email, PDO::PARAM_STR);
        $stmt->bindParam(':password_hash', $password_hash, PDO::PARAM_STR, 40);
